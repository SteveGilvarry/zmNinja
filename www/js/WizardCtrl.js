...
angular.module('zmApp.controllers').controller('zmApp.WizardCtrl', ['$scope', '$rootScope', '$ionicModal', 'NVR', '$ionicSideMenuDelegate', '$ionicHistory', '$state', '$ionicPopup', 'SecuredPopups', '$http', '$q', 'zm', '$ionicLoading', 'WizardHandler', '$translate', '$cookies', function ($scope, $rootScope, $ionicModal, NVR, $ionicSideMenuDelegate, $ionicHistory, $state, $ionicPopup, SecuredPopups, $http, $q, zm, $ionicLoading, WizardHandler, $translate, $cookies) {
  ...
  function login(url, zmusername, zmpassword) {
    var d = $q.defer();
    $http({
      method: 'post', // post requires csrf, which should be ok because of the action
  ...
  function wizardLogin(url, zmusername, zmpassword) {
    var d = $q.defer();
    var loginAPI = url + '/api/host/login.json';
    NVR.debug("Inside wizardLogin: will try "+loginAPI + zmusername +zmpassword);
      ...
      .then(function (textsucc) {
        ...
        try {
          ...
          if (!succ.version) {
            ...
            loginWebScrape(url, zmusername, zmpassword)
              .then(function (succ) {
  ...
  function validateData() {
    ...
    if ($scope.wizard.usezmauth) {
      zmusername = $scope.wizard.zmuser;
      zmpassword = $scope.wizard.zmpassword;
    }
    ...
    logout(u)
      .then(function (ans) {
        // login now
        $ionicLoading.hide();
        NVR.log("zmWizard: logging in with " + u + " " + zmusername);
        ...
        wizardLogin(u, zmusername, zmpassword)
          .then(function (success) {
            $ionicLoading.hide();
