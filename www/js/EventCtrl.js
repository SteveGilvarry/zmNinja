...
/* jslint browser: true*/
/* global saveAs, cordova,StatusBar,angular,console,moment, MobileAccessibility, gifshot, ReadableStream , GifWriter, NeuQuant */
...
angular.module('zmApp.controllers')
  ...
  .controller('zmApp.EventCtrl', ['$scope', '$rootScope', 'zm', 'NVR', 'message', '$ionicSideMenuDelegate', '$timeout', '$interval', '$ionicModal', '$ionicLoading', '$http', '$state', '$stateParams', '$ionicHistory', '$ionicScrollDelegate', '$ionicPlatform', '$ionicSlideBoxDelegate', '$ionicPosition', '$ionicPopover', '$ionicPopup', 'EventServer', '$sce', '$q', 'carouselUtils', '$translate', '$ionicListDelegate', 'ionPullUpFooterState', 'SecuredPopups', 'MediaHelper', '$window', function ($scope, $rootScope, zm, NVR, message, $ionicSideMenuDelegate, $timeout, $interval, $ionicModal, $ionicLoading, $http, $state, $stateParams, $ionicHistory, $ionicScrollDelegate, $ionicPlatform, $ionicSlideBoxDelegate, $ionicPosition, $ionicPopover, $ionicPopup, EventServer, $sce, $q, carouselUtils, $translate, $ionicListDelegate, ionPullUpFooterState, SecuredPopups, MediaHelper, $window) {
    ...
    function SaveError(e) {
      ...
      if (e) {
        try {
          NVR.log("Error saving image: " + JSON.stringify(e));
        } catch (ex) {
...
// javascript
    function saveNow(imgsrc, eid) {
      ...
      var album = 'zmNinja';
      var fname = 'zmNinja-eid-' + eid + '.jpg';
      var url = imgsrc;

      // Show once up-front; update as needed; hide on completion/error
      $ionicLoading.show({
      ...
      MediaHelper.savePhotoFromUrl(url, {
        ...
        // Desktop: fetch, then present a download link; no initial hide
        fetch(url)
          .then(function (resp) { return resp.blob(); })
          .then(function (blob) {
            $ionicLoading.hide(); // hide before showing the popup
            var objUrl = window.URL.createObjectURL(blob);
            var dlName = 'zmninja-eid-' + eid + '.jpg';
            $rootScope.zmPopup = SecuredPopups.show('alert', {
              title: $translate.instant('kNote'),
              template: $translate.instant('kDownloadVideoImage') +
                "<br/><br/><center><a href='" + objUrl +
                "' class='button button-assertive icon ion-android-download' download='" + dlName + "'>" +
                " " + $translate.instant('kDownload') + "</a></center>",
              okText: $translate.instant('kDismiss'),
              okType: 'button-stable'
            });

            $rootScope.zmPopup.then(function () {
    ...
    $scope.downloadFileToDevice = function (path, eid) {
      ...
      MediaHelper.saveVideoFromUrl(path, {
        fileName: fileName,
        album: album,
        headers: fetchOptions.headers,
        ...
        .then(function () {
          NVR.setAwake(false);
          $ionicLoading.hide();
          NVR.debug("Video saved successfully");
        })
        .catch(function (err) {
          NVR.setAwake(false);
          $ionicLoading.hide();
          NVR.log("Video save error=" + JSON.stringify(err));
          $ionicLoading.show({
    ...
    $scope.permissionsDownload = function (e) {
      if ($rootScope.platformOS == 'desktop') {
        ...
        MediaHelper.requestPermissions()
          ...
          .catch(function (err) {
            NVR.log("ERROR with saving permissions " + JSON.stringify(err));
            $ionicLoading.show({
              template: $translate.instant('kErrorSave'),
              noBackdrop: true,
              duration: 2000
              ...
              .then(function (img) {
                ...
                pull().then(function (chunks) {
                  ...
                  if ($rootScope.platformOS == 'desktop') {
                    ...
                    var album = "zmNinja";
                    var fileName = e.Event.Id + "-video.gif";

                    MediaHelper.saveBlobAsPhoto(blob, {
                      fileName: fileName,
                      album: album
                    })
                      .then(function () {
                        $rootScope.isDownloading = false;
                        $ionicLoading.hide();
                        NVR.debug("GIF saved successfully");
                      })
                      .catch(function (err) {
                        $rootScope.isDownloading = false;
                        $ionicLoading.hide();
                        NVR.debug("Saving GIF error=" + JSON.stringify(err));
                        $ionicLoading.show({
                          template: $translate.instant('kErrorSave'),
                          noBackdrop: true,
                          duration: 2000
            ...
            gifshot.createGIF({
              ...
              if (!obj.error) {
                ...
                if ($rootScope.platformOS == 'desktop') {
                  ...
                  NVR.debug("Saving GIF via Capacitor media plugin");
                  var album = "zmNinja";
                  var fileName = e.Event.Id + "-video.gif";
                  MediaHelper.savePhotoFromBase64(obj.image, {
                    fileName: fileName,
                    album: album
                  })
                    .then(function () {
